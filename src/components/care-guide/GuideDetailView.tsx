    ...
    <div className="flex flex-col gap-5">
      <div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 text-xs font-medium text-slate-400">
            <ClockIcon className="h-3.5 w-3.5" /> 예상 읽기 시간 {guide.readMinutes}분
          </span>
        </div>
      ...
      {guide.mustCheck && (
        <div className="flex gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-relaxed text-amber-900">{guide.mustCheck}</p>
          </div>
        </div>
      )}

      {guide.beforeCare && <Section title="검사·치료 전">{guide.beforeCare}</Section>}
      {guide.dayOfCare && <Section title="검사·치료 당일">{guide.dayOfCare}</Section>}
      {guide.afterCare && <Section title="검사·치료 후">{guide.afterCare}</Section>}

      {guide.precautions && (
        <div className="flex gap-2.5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3.5">
          <WarningIcon className="mt-0.5 h-4 w-4 shrink-0 text-rose-600" />
          <div>
            <p className="text-xs font-bold text-rose-800">주의사항</p>
            <p className="mt-1 text-sm leading-relaxed text-rose-900">{guide.precautions}</p>
          </div>
        </div>
      )}
      ...
      {variant === "staff" ? (
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-4 text-xs text-slate-400">
          <span>버전 {guide.version}{guide.approvedBy ? ` · ${guide.approvedBy} 승인` : ""}{guide.approvedAt ? ` (${guide.approvedAt})` : ""}</span>
